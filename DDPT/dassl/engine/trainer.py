...
class TrainerBase:
    ...
    def save_model(
        ...
        Args:
            ...
            is_best (bool) : if this is true, then the checkpoint will be copied and named 'model-best.pth.tar'
            val_result (str) : another parameter which goes into the state we want to save
    ...
    def resume_model_if_exist(self, directory):
        '''
        SO WHAT THIS FUNCTION DOES IS UPDATES THE STATE OF THE MODEL, OPTIM AND SCHED
        FROM THE SAVED DIRECTORY FILE, AND RETURNS THE EPOCH (INT) TO START AT 
        '''
        names = self.get_model_names()
        file_missing = False
    ...
    def load_model(self, directory, epoch=None):
        '''
        SO AT THIS STATE, ASSUMING THAT THE TRAINING IS DONE, THIS JUST LOADS EACH 
        MODEL WITH THE STATE FROM THE SAVED DIRECTORY
        '''
        if not directory:
            print(
    ...
    def set_model_mode(self, mode="train", names=None):
        '''
        BASED ON THE INPUT, SETS ALL THE MODELS whose names are passed in (OR ALL) TO TRAIN OR EVAL MODE
        '''
    ...
    def update_lr(self, names=None):
        '''
            Again we do this operation of updating the lr by doing step over the scheduler
            (for the model names that are passed in OR just all the models)
        '''
        names = self.get_model_names(names)
...
class SimpleTrainer(TrainerBase):
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            input, label = self.parse_batch_test(batch) #SIMPLE FUNCTION JUST TO GET THE IMG AND LABELS SEPARATE AND TO CUDA
