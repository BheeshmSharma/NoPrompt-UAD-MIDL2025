...
class VPT(TrainerX):
    ...
    def get_heatmap(self):
        ...
        x= self.model.image_encoder.conv1(test_image)
        x = x.reshape(x.shape[0], x.shape[1], -1) 
        x = x.permute(0, 2, 1) 
        x = torch.cat([self.model.image_encoder.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1) 
        x = x +self.model.image_encoder.positional_embedding.to(x.dtype)
        x = self.model.image_encoder.ln_pre(x)
        x = x.permute(1, 0, 2)
        ...
        torch.save(x1, 'hidden.pt')
        torch.save(test_image, 'image.pt')
